...
NUM_FILES = 20
LOAD_MAX_EPOCH = 250 # Kolik načíst ze souboru
PLOT_MAX_EPOCH = 200 # Kolik budeme vykreslovat po ořezání (250 - 50)
...
def get_detailed_data(base_path, num_files, max_epoch):
    """
    Načte data. Vrací slovník {metoda: matice (files x epochs)}.
    """
    results = {m: np.zeros((num_files, max_epoch + 1)) for m in METHODS_ORDER}
...
def cut_data_matrix(data_dict):
    """
    Vyřízne sloupce 100 až 149 z matic a zbytek spojí.
    Výsledná matice se zmenší o 50 epoch.
    """
    new_data = {}
    for method, matrix in data_dict.items():
        # matrix shape: (num_files, 251)
        # Vezmeme 0..99 a 150..konec
        part1 = matrix[:, :100] 
        part2 = matrix[:, 150:] 
        
        # Spojíme je dohromady (axis=1 je směr epoch)
        new_data[method] = np.concatenate((part1, part2), axis=1)
    return new_data

def plot_with_std(ax, detailed_data, event_text):
    # Zjistíme délku osy X dynamicky z dat (mělo by to být 201)
    # detailed_data[METHODS_ORDER[0]].shape[1] vrátí počet sloupců
    ...
    for method in METHODS_ORDER:
        ...
        # 1. Výpočet průměru a směrodatné odchylky
        mean_vals = np.mean(matrix, axis=0)
        std_vals = np.std(matrix, axis=0)
        
        # 2. Vyhlazení (Smoothing)
        mean_smoothed = pd.Series(mean_vals).rolling(window=20, center=True, min_periods=1, win_type='gaussian').mean(std=5)
        std_smoothed = pd.Series(std_vals).rolling(window=20, center=True, min_periods=1, win_type='gaussian').mean(std=5)
    ...
    # --- ČÁRA A TEXT POSUNUTY NA 100 ---
    ax.axvline(x=100, color='black', linestyle='--', linewidth=1.5, alpha=0.7)
    
    # Vypíšeme text specifický pro tento graf
    if event_text:
        ax.text(100 + 2, 1.05, event_text, fontsize=14, color='black')

# --- HLAVNÍ PROCES ---

# 1. Načtení dat (plný rozsah do 250)
...
# Voláme funkci s konkrétním popiskem pro každý graf
...
ax3.set_xlabel("Epochs", fontsize=14)
# Nastavíme limit osy X na 200 (protože jsme 50 uřízli)
