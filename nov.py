...
def plot_aabb(ax, min_p, max_p, color='gray', alpha=0.3):
    x_min, y_min, z_min = min_p
    x_max, y_max, z_max = max_p

    # Definice vrcholů
    vertices = [
        [[x_min, y_min, z_min], [x_max, y_min, z_min], [x_max, y_max, z_min], [x_min, y_max, z_min]], # Dolní stěna
        [[x_min, y_min, z_max], [x_max, y_min, z_max], [x_max, y_max, z_max], [x_min, y_max, z_max]], # Horní stěna
        [[x_min, y_min, z_min], [x_min, y_max, z_min], [x_min, y_max, z_max], [x_min, y_min, z_max]], # Levá
        [[x_max, y_min, z_min], [x_max, y_max, z_min], [x_max, y_max, z_max], [x_max, y_min, z_max]], # Pravá
        [[x_min, y_min, z_min], [x_max, y_min, z_min], [x_max, y_min, z_max], [x_min, y_min, z_max]], # Přední
        [[x_min, y_max, z_min], [x_max, y_max, z_min], [x_max, y_max, z_max], [x_min, y_max, z_max]], # Zadní
    ]

    # Vykreslení stěn
    poly = Poly3DCollection(vertices, alpha=alpha, facecolors=color, edgecolors='black')
...
def main():
    ...
    while True:
        ...
        while True:
            ...
            for pole in vybrane_pole:
                ...
                if np.linalg.norm(new_pos - prev_pos) > threshold:
                    ...
                    if choice == 'exploration' or choice == 'exploration_path':
        ...
        np.savetxt("nov_21.txt", results_array, fmt="%s")
